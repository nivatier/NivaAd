...
function NotFoundComponent() {
  ...
  useEffect(() => {
    if (loading) return;
    // Authenticated users land on the app dashboard, guests on the homepage
    router.navigate({ to: isAuthed ? "/app" : "/", replace: true });
  }, [loading, isAuthed, router]);

  // Show nothing while redirecting — avoids a flash of the 404 page
  return null;
}
